...
export default function ProfilePage() {
  ...
  return <main className="min-h-screen bg-[#f3efe7] px-4 py-8 sm:px-6 sm:py-10"><div className="mx-auto max-w-5xl overflow-hidden rounded-[2rem] bg-[#fffdf8] shadow-xl">
    ...
    <div className="p-6 sm:p-10">
      ...
      <div className="mb-10 rounded-2xl border border-blue-100 bg-[#f3eadc] p-5"><p className="font-black text-[#17130e]">💡 What matters most?</p><p className="mt-1 text-sm leading-6 text-[#6f675b]">Name + role + city + experience + skills first. Your portfolio is what will make the profile stand out.</p></div>
      <section><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">1 · About you</p><h2 className="mt-2 text-2xl font-black">👤 Basic information</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><Field label="Full name *" value={fullName} setValue={setFullName} placeholder="Your full name" /><label><span className="mb-2 block text-sm font-bold">Email</span><input value={email} readOnly className="w-full rounded-xl border border-slate-100 bg-[#f6f1e7] p-4 text-[#7b7265]" /></label><Field label="Phone number" value={phone} setValue={setPhone} placeholder="+91 XXXXX XXXXX" /><Field label="Current city" value={city} setValue={setCity} placeholder="Mumbai, Delhi, Bengaluru..." /></div></section>
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">2 · Education</p><h2 className="mt-2 text-2xl font-black">🎓 Education</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><Field label="Institute name" value={institute} setValue={setInstitute} placeholder="Hotel management institute / college" /><Field label="Course" value={course} setValue={setCourse} placeholder="BHM, Culinary Arts, Bakery..." /><Field label="Graduation year" value={graduationYear} setValue={setGraduationYear} placeholder="2025" /><Field label="CGPA / Percentage" value={score} setValue={setScore} placeholder="8.2 CGPA / 82%" /></div></section>
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">3 · Career</p><h2 className="mt-2 text-2xl font-black">💼 What do you do?</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><Field label="Role / specialization *" value={role} setValue={setRole} placeholder="Commis Chef, Barista, Front Office..." /><Field label="Preferred city" value={preferredCity} setValue={setPreferredCity} placeholder="Where would you like to work?" /><label><span className="mb-2 block text-sm font-bold">Experience (years)</span><input type="number" min="0" step="0.5" value={experience} onChange={e => setExperience(e.target.value)} className="w-full rounded-xl border border-[#d8cdbb] p-4" placeholder="0 for fresher" /></label><Field label="Expected salary (optional)" value={salary} setValue={setSalary} placeholder="₹25,000 / month" /></div></section>
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">4 · Skills</p><h2 className="mt-2 text-2xl font-black">⭐ Your skills</h2><p className="mt-1 text-sm text-[#7b7265]">Add the skills employers might search for. Separate them with commas.</p><textarea value={skills} onChange={e => setSkills(e.target.value)} placeholder="Bakery, Continental, Latte Art, Housekeeping, Front Office..." className="mt-5 h-32 w-full rounded-xl border border-[#d8cdbb] p-4" /></section>
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">5 · Your story</p><h2 className="mt-2 text-2xl font-black">📝 About me</h2><textarea value={about} onChange={e => setAbout(e.target.value)} placeholder="Tell employers about your experience, strengths and the kind of hospitality work you enjoy..." className="mt-5 h-40 w-full rounded-xl border border-[#d8cdbb] p-4" /></section>
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">6 · Resume</p><h2 className="mt-2 text-2xl font-black">📄 Resume</h2><div className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-[#f6f1e7] p-6"><p className="font-bold text-slate-700">Resume is optional for now.</p><p className="mt-1 text-sm text-[#7b7265]">Save your profile first. We will add a proper resume upload with file validation in the next production step.</p></div></section>
      <section id="my-work" className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">7 · My Work</p><h2 className="mt-2 text-2xl font-black">📸 Show your work</h2><p className="mt-1 text-sm text-[#7b7265]">Add photos or videos of dishes, coffee, bakery, hotel operations, projects and achievements.</p><PortfolioGallery /></section>
