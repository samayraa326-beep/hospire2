...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data) {
        ...
        setScore(data.score || "");
        setRole(data.hospitality_role || "");
        setPreferredCity(data.preferred_city || data.city || "");
        setExperience(data.experience_years != null ? String(data.experience_years) : "");
        setSalary(data.expected_salary || "");
  ...
  async function saveProfile() {
    ...
    if (experienceNumber !== null && (!Number.isFinite(experienceNumber) || experienceNumber < 0)) {
      setError("Experience must be a valid number of years."); return;
    }
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-8 sm:px-6 sm:py-10">
      <div className="mx-auto max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-xl">
        <header className="bg-gradient-to-br from-blue-950 via-slate-950 to-slate-900 p-7 text-white sm:p-10">
          <Link href="/" className="text-2xl font-black">Hospire</Link>
          <p className="mt-8 text-sm font-black uppercase tracking-[.2em] text-blue-300">Candidate profile</p>
          <h1 className="mt-3 text-3xl font-black sm:text-4xl">Build a profile employers remember.</h1>
          <p className="mt-3 max-w-2xl text-slate-300">You do not need to complete everything today. Start with the essentials, save them, then add your best work.</p>
        </header>

        <div className="p-6 sm:p-10">
          {saved && <div className="mb-8 rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><p className="text-lg font-black text-emerald-900">🎉 Profile saved successfully!</p><p className="mt-1 text-sm text-emerald-800">Your next best step is to add 2–3 examples of your work.</p><div className="mt-4 flex flex-wrap gap-3"><a href="#my-work" className="rounded-xl bg-blue-700 px-4 py-3 text-sm font-black text-white">Add My Work →</a><Link href={`/profile/${profileId}`} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-900">Preview My Profile</Link><Link href="/discover" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-900">Explore Talent</Link></div></div>}
          ...
          <section><p className="text-sm font-black uppercase tracking-widest text-blue-700">1 · About you</p><h2 className="mt-2 text-2xl font-black">👤 Basic information</h2><div className="mt-5 grid gap-5 md:grid-cols-2">
            <Field label="Full name *" value={fullName} setValue={setFullName} placeholder="Your full name" />
            ...
            <Field label="Phone number" value={phone} setValue={setPhone} placeholder="+91 XXXXX XXXXX" />
            <Field label="Current city" value={city} setValue={setCity} placeholder="Mumbai, Delhi, Bengaluru..." />
          </div></section>

          <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-blue-700">2 · Education</p><h2 className="mt-2 text-2xl font-black">🎓 Education</h2><div className="mt-5 grid gap-5 md:grid-cols-2">
            <Field label="Institute name" value={institute} setValue={setInstitute} placeholder="Hotel management institute / college" />
            <Field label="Course" value={course} setValue={setCourse} placeholder="BHM, Culinary Arts, Bakery..." />
            <Field label="Graduation year" value={graduationYear} setValue={setGraduationYear} placeholder="2025" />
            <Field label="CGPA / Percentage" value={score} setValue={setScore} placeholder="8.2 CGPA / 82%" />
          </div></section>

          <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-blue-700">3 · Career</p><h2 className="mt-2 text-2xl font-black">💼 What do you do?</h2><div className="mt-5 grid gap-5 md:grid-cols-2">
            <Field label="Role / specialization *" value={role} setValue={setRole} placeholder="Commis Chef, Barista, Front Office..." />
            <Field label="Preferred city" value={preferredCity} setValue={setPreferredCity} placeholder="Where would you like to work?" />
            ...
            <Field label="Expected salary (optional)" value={salary} setValue={setSalary} placeholder="₹25,000 / month" />
          </div></section>
...
function Field({ label, value, setValue, placeholder }: { label: string; value: string; setValue: (value: string) => void; placeholder: string }) {
  return <label><span className="mb-2 block text-sm font-bold">{label}</span><input value={value} onChange={e => setValue(e.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-slate-200 p-4 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" /></label>;
}
