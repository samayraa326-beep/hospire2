...
{resumeStatus && <p className="mt-3 text-sm text-[#d8c28d]">{resumeStatus}</p>}
      ...
      <section className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">6 · Education proof</p><h2 className="mt-2 text-2xl font-black">📄 Resume</h2><div className="mt-5 rounded-2xl border border-[#d8cdbb] bg-[#f6f1e7] p-6"><p className="font-bold text-[#17130e]">{resumeUrl ? "Resume uploaded" : "No resume uploaded yet"}</p><p className="mt-1 text-sm text-[#7b7265]">{resumeUrl ? resumeName : "Upload a PDF above to pre-fill your profile."}</p>{resumeUrl && <a href={resumeUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-semibold text-[#9a7337]">View resume →</a>}</div></section>
      <section id="my-work" className="mt-12"><p className="text-sm font-black uppercase tracking-widest text-[#9a7337]">7 · My Work</p><h2 className="mt-2 text-2xl font-black">📸 Show your work</h2><p className="mt-1 text-sm text-[#7b7265]">Let your work speak for you — photos, videos, projects and achievements.</p><PortfolioGallery /></section>
