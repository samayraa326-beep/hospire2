    ...
    <main className="min-h-screen bg-slate-950 px-5 py-8 sm:px-6 sm:py-10">
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl overflow-hidden rounded-[2rem] bg-white shadow-2xl lg:grid-cols-2">
        <section className="hidden bg-gradient-to-br from-blue-950 via-slate-950 to-slate-900 p-12 text-white lg:flex lg:flex-col lg:justify-between">
          <div><div className="text-3xl font-black tracking-tight">Hospire</div><p className="mt-2 text-sm font-semibold text-blue-200">Hospitality careers, built on trust.</p></div>
        ...
        <section className="p-6 sm:p-10 lg:p-14">
          <div className="mb-8 lg:hidden"><div className="text-3xl font-black">Hospire</div><p className="mt-1 text-sm text-slate-500">Hospitality careers, built on trust.</p></div>
          <div className="mx-auto max-w-xl">
            ...
            <button type="button" onClick={handleGoogle} disabled={googleLoading} className="mt-8 flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 px-5 py-4 font-black text-slate-900 transition hover:border-slate-300 hover:bg-slate-50 disabled:opacity-60">
              <span className="text-lg font-black">G</span>{googleLoading ? "Connecting to Google..." : "Continue with Google"}
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {mode === "signup" && <>
                <div className="grid gap-3 sm:grid-cols-2">
                  <button type="button" onClick={() => setRole("candidate")} className={`rounded-2xl border p-4 text-left transition ${role === "candidate" ? "border-blue-600 bg-blue-50 ring-2 ring-blue-100" : "border-slate-200"}`}><span className="text-xl">👨‍🍳</span><span className="mt-2 block font-black">Looking for work</span></button>
                ...
                <label className="block"><span className="mb-2 block text-sm font-bold">Full name</span><input value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" placeholder="Your full name" /></label>
              </>}
              ...
              <div className={mode === "signup" ? "grid gap-5 sm:grid-cols-2" : ""}>
                <label className="block"><span className="mb-2 block text-sm font-bold">Password</span><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete={mode === "login" ? "current-password" : "new-password"} className="w-full rounded-xl border border-slate-200 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" placeholder={mode === "login" ? "Your password" : "Minimum 6 characters"} /></label>
                {mode === "signup" && <label className="block"><span className="mb-2 block text-sm font-bold">Confirm password</span><input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required autoComplete="new-password" className="w-full rounded-xl border border-slate-200 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" placeholder="Repeat password" /></label>}
              </div>
